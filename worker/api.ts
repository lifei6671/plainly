...
const signJwt = async (payload: Record<string, unknown>, secret: string, expiresSec?: number) => {
...
const createSession = async (db, userId, refreshToken, expiresAt, meta: SessionMeta = {}) => {
  const now = Date.now();
...
const createCategory = async (db, userId, name, options: CreateCategoryOptions = {}) => {
...
const deleteCategory = async (db, userId, categoryUuid, options: DeleteCategoryOptions = {}) => {
  const row = await getCategoryRowByUuid(db, userId, categoryUuid);
  if (!row) return;
...
const searchDocumentsByTokens = async (db, userId, tokens, options: SearchDocumentOptions = {}) => {
