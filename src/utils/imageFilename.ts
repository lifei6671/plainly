...
type Resolver = (arg?: string) => string;

function pad2(n: number) {
  return String(n).padStart(2, "0");
}

function sanitizeSegment(s: unknown) {
...
function getBaseName(filename: unknown) {
...
function getExt(filename: unknown, fallbackExt = "webp") {
 ...
 */
export default function renderObjectName(template: string, ctx: RenderObjectContext = {}) {
  ...
  // 只替换 ${...}，并且只允许白名单变量
  const out = String(template || "").replace(/\$\{([^}]+)\}/g, (_, expr: string) => {
