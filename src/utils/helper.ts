...
type StyleElementWithSheet = HTMLStyleElement & {
  styleSheet?: {
    cssText: string;
  };
};
...
declare global {
  interface Window {
    MathJax?: {
      texReset?: () => void;
      typesetClear?: () => void;
      typesetPromise?: () => void | Promise<unknown>;
    };
  }
}
...
export const replaceStyle = (id, css) => {
  const style = document.getElementById(id) as StyleElementWithSheet | null;
  if (!style) {
    return;
  }
  try {
 ...
 * @returns {string}
 */
export function extractVisibleText(markdown, opts: VisibleTextOptions = {}) {
 ...
 */
export function countVisibleChars(markdown, opts: VisibleTextOptions = {}) {
