...
class Content {
  ...
  setDocumentCategoryUuid = (documentCategoryUuid) => {
    const nextValue = documentCategoryUuid ? String(documentCategoryUuid).replace(/-/g, "") : DEFAULT_CATEGORY_UUID;
...
// 如果为空先把数据放进去
if (getConfigSync(CONTENT, null) === null) {
  setConfigSync(CONTENT, TEMPLATE.content);
}
if (getConfigSync(DOCUMENT_ID, null) === null) {
  const legacy = getConfigSync(DOCUMENT_UUID, "");
  setConfigSync(DOCUMENT_ID, legacy ? String(legacy).replace(/-/g, "") : "");
}
if (getConfigSync(DOCUMENT_NAME, null) === null) {
  setConfigSync(DOCUMENT_NAME, "未命名.md");
}
if (getConfigSync(DOCUMENT_UPDATED_AT, null) === null) {
  setConfigSync(DOCUMENT_UPDATED_AT, "0");
}
if (getConfigSync(DOCUMENT_CATEGORY_ID, null) === null) {
  const legacy = getConfigSync(DOCUMENT_CATEGORY_UUID, DEFAULT_CATEGORY_UUID);
  setConfigSync(DOCUMENT_CATEGORY_ID, legacy ? String(legacy).replace(/-/g, "") : DEFAULT_CATEGORY_UUID);
}
if (getConfigSync(DOCUMENT_CATEGORY_NAME, null) === null) {
...
const storedDocumentId = String(getConfigSync(DOCUMENT_ID, "")) || String(getConfigSync(DOCUMENT_UUID, "") || "");
...
store.documentCategoryId = Number.isFinite(Number(storedCategoryId)) ? Number(storedCategoryId) : DEFAULT_CATEGORY_ID;
