...
import type {QiniuOSSConfig} from "./types";

type QiniuOSSState = {
  imageHosting: QiniuOSSConfig;
  link: string;
};
...
class QiniuOSS extends Component {
  defaultImageHosting: QiniuOSSConfig = {
    bucket: "",
    region: "",
    accessKey: "",
    secretKey: "",
  ...
  async componentDidMount() {
    const imageHosting = await loadHostingConfig<QiniuOSSConfig>({
  ...
  persistConfig = (nextConfig: QiniuOSSConfig) => {
  ...
  accessKeyChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const {imageHosting} = this.state;
  ...
  secretKeyChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const {imageHosting} = this.state;
  ...
  linkChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  namespaceChange = ({target: {value}}: React.ChangeEvent<HTMLInputElement>) => {
    const {imageHosting} = this.state;
