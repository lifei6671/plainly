...
import type {AliOSSConfig} from "./types";

type AliOSSState = {
  imageHosting: AliOSSConfig;
};
...
class AliOSS extends Component {
  defaultImageHosting: AliOSSConfig = {
    region: "",
    accessKeyId: "",
    accessKeySecret: "",
    bucket: "",
  ...
  async componentDidMount() {
    const imageHosting = await loadHostingConfig<AliOSSConfig>({
  ...
  persistConfig = (nextConfig: AliOSSConfig) => {
  ...
  regionChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  accessKeySecretChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const {imageHosting} = this.state;
