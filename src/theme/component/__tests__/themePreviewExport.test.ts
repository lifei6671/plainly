...
describe("component theme production preview and WeChat export", () => {
  it("keeps the existing theme index contract", () => {
    ...
    const zenWhitespaceIndex = themes.findIndex((theme) => theme.id === ZEN_WHITESPACE_THEME.id);
  ...
  it("runs the Zen theme through the standard parser, component preview, and export", () => {
    const {rawHtml, preview, exportHtml} = renderThemePreview(ZEN_WHITESPACE_THEME.id, TECH_DEPTH_TECHNICAL_ARTICLE);
    const parsed = toRoot(rawHtml);
  ...
  it("keeps WeChat parser fenced-code structure outside Zen list wrappers through export", () => {
    const {rawHtml, preview, exportHtml} = renderThemePreview(
      ZEN_WHITESPACE_THEME.id,
      TECH_DEPTH_TECHNICAL_ARTICLE,
      "wechat",
    );
    const parsed = toRoot(rawHtml);
