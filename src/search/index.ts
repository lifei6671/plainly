...
declare global {
  interface Window {
    __DATA_STORE_MODE__?: string;
    __DATA_STORE_USER_ID__?: number;
    __CURRENT_USER_ID__?: number;
  }
}
...
function resolveScope(): SearchScope {
...
async function buildRawDocsFromArticles(): Promise<SearchRawDoc[]> {
  const db = await openArticlesDb();
  if (!db) return [];
 ...
 */
export async function initSearch(rawDocs: SearchRawDoc[], opts: InitSearchOptions = {}) {
  ...
  const docs: SearchDoc[] = await mapLimit(rawDocs, concurrency, async (d) => ({
    id: d.id,
    title: d.title || "",
 ...
 * @param {string} q
 */
export function search(idx: lunr.Index, q: string) {
